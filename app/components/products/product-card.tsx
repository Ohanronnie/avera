...
import { Heart, Star, ShoppingCart, Bookmark, BookmarkCheck } from "lucide-react-native";
  ...
  return (
    ...
    >
      ...
      <View className="relative h-48 bg-gray-100 dark:bg-white/5">
        ...
        {/* Product Image */}
        <Image
          source={imageUrl ? { uri: imageUrl } : require("@/assets/images/shoe.jpg")}
      ...
      <View className="p-3 space-y-2">
        {/* Title */}
        <Text className="text-sm font-semibold text-gray-900 dark:text-gray-200" numberOfLines={1}>
          {title}
        </Text>

        {/* Price */}
        <View className="flex-row items-baseline">
          <Text className="text-lg font-bold text-gray-900 dark:text-white mr-2">₦{price.toLocaleString()}</Text>
          {originalPrice && (
        ...
        <View className="flex-row mb-2 items-center">
          ...
          <Text className="ml-1 text-xs text-gray-500 dark:text-gray-400">{location}</Text>
        </View>
        ...
        {condition && (
          <View className="bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded-md self-start mt-1 border border-gray-200 dark:border-white/5">
            <Text className="text-gray-600 dark:text-gray-400 text-[10px] font-bold uppercase tracking-tight">{condition}</Text>
          </View>
        )}
      </View>
    </Pressable>
