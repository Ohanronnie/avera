...
>(function InputIcon({ className, size, ...props }, ref) {
  ...
  const parentSize = styleContext?.size ?? 'md';
...
const InputField = React.forwardRef<
  ...
  IInputFieldProps
>(function InputField({ className, type, secureTextEntry, ...props }, ref) {
  const styleContext = useStyleContext(SCOPE);
  const parentVariant = styleContext?.variant ?? 'outline';
  const parentSize = styleContext?.size ?? 'md';
  const resolvedSecureTextEntry =
    secureTextEntry ?? (type === 'password' ? true : undefined);
  const fieldClassName = inputFieldStyle({
    parentVariants: {
