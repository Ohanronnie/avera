...
export default function P2POrderScreen() {
  ...
  const canContinue = !!listing && amountValue > 0 && !isBelowLimit && !isAboveLimit;
    ...
    return (
      <SafeAreaView className="flex-1 bg-[#050505]" edges={["top"]}>
        <View className="flex-row items-center justify-between px-5 py-4">
          ...
          <Text className="text-lg font-bold text-white">P2P order</Text>
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="mt-5 text-xl font-black text-white">Offer not found</Text>
          <Text className="mt-2 text-center text-sm leading-6 text-gray-400">
            This P2P offer may no longer be available. Go back and choose another seller.
          </Text>
          <Pressable
            onPress={() => router.replace("/p2p")}
            className="mt-8 h-14 w-full items-center justify-center rounded-2xl bg-brand"
          >
            <Text variant="none" className="font-bold text-white">Back to offers</Text>
          </Pressable>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#050505]" edges={["top"]}>
      ...
      >
        <View className="rounded-[32px] border border-white/5 bg-[#111214] p-5">
          <Text className="text-sm font-bold uppercase tracking-widest text-gray-500">
            Amount you want to {side === "buy" ? "buy" : "sell"}
          </Text>
          <View className="mt-4 flex-row items-center rounded-3xl border border-white/10 bg-white/5 px-4">
            <Text variant="none" className="text-2xl font-black text-brand">₦</Text>
            <TextInput
              value={amount}
              ...
              className="h-16 flex-1 px-3 py-0 text-3xl font-black leading-9 text-white"
          ...
          <View className="mt-4 rounded-2xl bg-brand/10 p-4">
            ...
            <Text className="mt-2 text-3xl font-black text-white">
              {cryptoAmount.toFixed(listing.asset === "USDT" ? 2 : 6)} {listing.asset}
            </Text>
          ...
          {(isBelowLimit || isAboveLimit) && (
            <View className="mt-4 flex-row items-start rounded-2xl border border-red-500/20 bg-red-500/10 p-3">
              ...
              <Text className="ml-2 flex-1 text-sm leading-5 text-red-300">
                Enter an amount between {formatNaira(listing.minLimit)} and {formatNaira(listing.maxLimit)}.
              </Text>
            </View>
          )}
        </View>
        ...
        <Pressable
          ...
          className="mt-5 rounded-3xl border border-white/5 bg-[#111214] p-5"
        >
            ...
            <View className="flex-1 flex-row items-center pr-4">
              ...
              <View className="ml-4 flex-1">
                <View className="flex-row items-center">
                  <Text className="text-base font-black text-white">{listing.seller}</Text>
                  {listing.verified && (
                    <Ionicons name="shield-checkmark" size={15} color="#2563EB" style={{ marginLeft: 6 }} />
                  )}
                </View>
        ...
        <View className="mt-5 rounded-3xl border border-white/5 bg-[#111214]">
          {[
            ...
            { label: "Estimated release", value: `${listing.speed} min after payment` },
          ].map((item, index, rows) => (
            <View
              key={item.label}
              className={`px-5 py-4 ${index !== rows.length - 1 ? "border-b border-white/5" : ""}`}
            >
              <Text className="text-xs font-bold uppercase tracking-widest text-gray-500">
                {item.label}
              </Text>
              <Text className="mt-2 text-base font-bold text-white">{item.value}</Text>
            </View>
          ))}
        </View>
        ...
        <View className="mt-5 rounded-3xl border border-brand/20 bg-brand/10 p-4">
          <View className="flex-row items-start">
            ...
            <Text className="ml-2 flex-1 text-sm leading-5 text-gray-300">
              Avera will hold the crypto in escrow while payment is completed. Only continue after checking the seller rate and limits.
            </Text>
          </View>
        </View>
          ...
          <Pressable
            ...
            onPress={() =>
              toast.show({
                ...
                description: "Next step is backend order creation and payment instructions.",
                variant: "info",
              })
            }
            className={`h-14 flex-1 items-center justify-center rounded-2xl ${canContinue ? "bg-brand" : "bg-white/10"}`}
          >
            <Text variant="none" className={`font-bold ${canContinue ? "text-white" : "text-gray-500"}`}>
              Continue
            </Text>
          </Pressable>
        </View>
